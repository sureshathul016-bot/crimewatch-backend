...
console.log(process.env.RESEND_API_KEY
  ...
  : '❌ RESEND_API_KEY not set in Railway variables');
...
app.post('/api/auth/send-otp', async (req, res) => {
  ...
  try {
    ...
    let userMsg = 'Failed to send OTP email. Please try again.';
    if (!process.env.RESEND_API_KEY) {
      userMsg = 'Email service not configured on server.';
    } else if (err.message.includes('Invalid API')) {
      userMsg = 'Email service authentication failed. Contact admin.';
    }
