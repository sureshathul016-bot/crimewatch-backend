...
app.post('/api/auth/register', async (req, res) => {
  const { name, username, password, pin, email } = req.body;
...
app.post('/api/auth/login', async (req, res) => {
  ...
  res.json({
    ...
    name: user.name, username: user.username, role: 'user', email: user.email || null, emailVerified: user.email_verified || false
  });
});
...
app.post('/api/reports', auth, upload.single('image'), async (req, res) => {
  ...
  const userRecord = await query('SELECT email_verified, email FROM users WHERE username=$1', [req.user.username]);
  const userRow = userRecord.rows[0];
  if (!userRow || !userRow.email_verified) {
    if (req.file) { try { fs.unlinkSync(req.file.path); } catch {} }
    return res.status(403).json({
      error: 'Email not verified. Please verify your email before submitting reports.',
      unverified: true
    });
  }
