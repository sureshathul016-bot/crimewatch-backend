...
transporter.verify((err) => {
  ...
  else console.log('✅ Gmail SMTP ready — OTPs will send to any email');
});
...
async function sendEmailOTP(to, otp) {
  ...
  if (!process.env.GMAIL_USER || !process.env.GMAIL_PASS) {
    throw new Error('GMAIL_USER and GMAIL_PASS must be set in Railway variables.');
  }
  const info = await transporter.sendMail({
    from: `"CrimeWatch" <${process.env.GMAIL_USER}>`,
    to,
...
async function initDB() {
  ...
  // Tracks how many reports each user submits per day
  await query(`
...
async function checkRateLimit(username) {
  const today = new Date().toISOString().split('T')[0]; // e.g. "2024-06-10"
  ...
  if (!result.rows.length) {
    // First report today — insert new row
    await query(
...
// ════════════════════════════════════════════
// SECURITY FEATURE 2: DUPLICATE DETECTION
// Checks if a very similar report already exists:
// Same user + same type + same location (within last 24 hours)
// ════════════════════════════════════════════
async function checkDuplicate(username, type, location, description) {
  // Strategy 1: Exact type + location (case-insensitive, trimmed)
  ...
  // Strategy 2: Same type + partial location match
  // Catches "Main St" vs "Main Street" or different capitalisation
  ...
  // Strategy 3: Same type + very similar description (5+ matching key words)
  // Catches copy-paste reports with only minor wording changes
...
app.post('/api/auth/register', async (req, res) => {
  const { name, username, password, pin, email, securityQuestion, securityAnswer } = req.body;
  if (!name || !username || !password) return res.status(400).json({ error: 'Missing fields' });
  if (!securityQuestion || !securityAnswer) return res.status(400).json({ error: 'Security question and answer are required.' });
  ...
  // Hash the security answer too for safety
  const hashedAnswer = await bcrypt.hash(securityAnswer.trim().toLowerCase(), 10);
  await query(
    'INSERT INTO users(name,username,password,pin,email,security_question,security_answer,email_verified) VALUES($1,$2,$3,$4,$5,$6,$7,$8)',
    [name, username.toLowerCase(), hashed, pin || null, email || null, securityQuestion, hashedAnswer, false]
  );
...
app.post('/api/auth/login', async (req, res) => {
  ...
  res.json({
    ...
    name: user.name, username: user.username, role: 'user', email: user.email || null, emailVerified: user.email_verified || false, securityQuestion: user.security_question || null
  });
});
...
app.post('/api/auth/send-otp', async (req, res) => {
  ...
  try {
    ...
    if (err.message.includes('Invalid login') || err.message.includes('535')) {
      userMsg = 'Email login failed — admin must update Gmail App Password.';
    } else if (err.message.includes('GMAIL_USER') || err.message.includes('GMAIL_PASS')) {
      userMsg = 'Email not configured on server.';
    }
...
app.post('/api/reports', auth, upload.single('image'), async (req, res) => {
  ...
  // SECURITY: Block unverified users from submitting reports
  ...
  if (!userRow || !userRow.email_verified) {
    ...
    return res.status(403).json({
      error: 'Identity not verified. Please answer your security question before submitting reports.',
      unverified: true
    });
  }
  ...
  try {
    ...
    if (!rateCheck.allowed) {
      // Delete uploaded file if rejected
      if (req.file) {
    ...
    // If rate limit check fails, allow the report through (fail open)
  }
  ...
  try {
    ...
    if (duplicate) {
      ...
      return res.status(409).json({
        error: `A similar report (${duplicate.id}) was already submitted with ${duplicate.reason} in the last 24 hours. Please check your existing reports.`,
        duplicate_id: duplicate.id
      });
    }
  } catch (err) {
    console.error('Duplicate check failed:', err.message);
    // If duplicate check fails, allow the report through (fail open)
  }

  // ── All checks passed — save report ──
...
// ── SECURITY: Endpoint to check remaining daily reports for a user ──
...
app.get('/api/auth/security-question', auth, async (req, res) => {
  const result = await query('SELECT security_question FROM users WHERE username=$1', [req.user.username]);
  const user = result.rows[0];
  if (!user || !user.security_question) return res.status(404).json({ error: 'No security question set.' });
  res.json({ question: user.security_question });
});

app.post('/api/auth/verify-security', auth, async (req, res) => {
  const { answer } = req.body;
  if (!answer) return res.status(400).json({ error: 'Answer required.' });
  const result = await query('SELECT security_answer FROM users WHERE username=$1', [req.user.username]);
  const user = result.rows[0];
  if (!user || !user.security_answer) return res.status(404).json({ error: 'No security question set.' });
  const match = await bcrypt.compare(answer.trim().toLowerCase(), user.security_answer);
  if (!match) return res.status(401).json({ error: 'Wrong answer. Please try again.' });
  // Mark as verified
  await query('UPDATE users SET email_verified=TRUE WHERE username=$1', [req.user.username]);
