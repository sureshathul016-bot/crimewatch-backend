...
transporter.verify((err) => {
  ...
  else console.log('✅ Brevo SMTP ready — OTPs will send to any email');
});
...
async function sendEmailOTP(to, otp) {
  console.log(`📧 Sending OTP to ${to} via Brevo...`);
  if (!process.env.BREVO_USER || !process.env.BREVO_PASS) {
    throw new Error('BREVO_USER and BREVO_PASS must be set in Railway variables.');
  }
  const info = await transporter.sendMail({
    from: `"CrimeWatch" <${process.env.BREVO_USER}>`,
    to,
...
app.post('/api/auth/send-otp', async (req, res) => {
  ...
  try {
    ...
    if (err.message.includes('BREVO_USER') || err.message.includes('BREVO_PASS')) {
