...
// ════════════════════════════════════════════
async function checkDuplicate(username, type, location) {
  const result = await query(
...
app.post('/api/reports', auth, upload.single('image'), async (req, res) => {
  ...
  try {
    const duplicateId = await checkDuplicate(req.user.username, type, location);
    if (duplicateId) {
      // Delete uploaded file if rejected
      if (req.file) {
        try { fs.unlinkSync(req.file.path); } catch {}
      ...
      return res.status(409).json({
        error: `A similar report (${duplicateId}) was already submitted for this type and location in the last 24 hours. Please check your existing reports.`,
